...
function LoginForm() {
  ...
  // Show error from NextAuth redirect (e.g. OAuthAccountNotLinked)
  useEffect(() => {
    const err = params.get("error");
    if (!err) return;
    const msg =
      err === "OAuthAccountNotLinked" ? "This email is linked to a different sign-in method. Try Google or use your password." :
      err === "CredentialsSignin"     ? "Invalid email or password." :
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        addToast({ title: "Welcome back 🌙", description: "Redirecting to your dashboard…", variant: "success" });
    ...
    <div className="min-h-screen flex items-center justify-center px-5 py-12 relative overflow-hidden"
      ...
      <div className={`relative z-10 w-full max-w-md transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-body text-white/50 text-sm">Sign in to create and share your Eid cards</p>
        </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full bg-white/5 border rounded-xl pl-10 pr-4 py-3 font-body text-sm text-white placeholder:text-white/25 transition-all duration-200 outline-none
                    ${errors.email ? "border-rose-500/60 focus:border-rose-400 focus:shadow-[0_0_0_3px_rgba(239,68,68,0.15)]" : "border-white/10 focus:border-amber-400/60 focus:shadow-[0_0_0_3px_rgba(251,191,36,0.12)]"}`}
                />
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="font-body text-sm font-semibold text-white/80">Password</label>
                <Link href="/forgot-password" className="font-body text-xs text-amber-400/70 hover:text-amber-400 transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full bg-white/5 border rounded-xl pl-10 pr-11 py-3 font-body text-sm text-white placeholder:text-white/25 transition-all duration-200 outline-none
                    ${errors.password ? "border-rose-500/60 focus:border-rose-400 focus:shadow-[0_0_0_3px_rgba(239,68,68,0.15)]" : "border-white/10 focus:border-amber-400/60 focus:shadow-[0_0_0_3px_rgba(251,191,36,0.12)]"}`}
                />
        ...
        <p className="text-center font-body text-sm text-white/40 mt-6">
          ...
          <Link href="/register" className="text-amber-400 hover:text-amber-300 font-semibold transition-colors">Create one free</Link>
        </p>
      </div>
    </div>
  );
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#0d0b2e" }}>
